...
fn main(){
	...
	if words_count < 10000 || words_count > 2000000 { panic!("Book is either too big or too small"); }
	...
	if matches.is_present("image") {
		...
		println!("{:?}", longest);
		if !b {println!("{}for most common used words...", "Looking ".color(TermColor::Cyan).bold())}
